...
/* ─────────────────────────────────────────────────────────────── */
export default function AuthModal({ onClose, onAuth, initialView = 'login' }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (view === 'login') {
        ...
        const data = await post('/forgot-password', { accountInput: email.trim() });
        if (data.email) {
          setEmail(data.email);
        }
        ...
        const data = await post('/verify-otp', { email: email.trim(), otp });
        ...
        if (!resetToken) throw new Error('Please verify the OTP again.');
      ...
      <div className="auth-modal">
        <div className="auth-panel">
          <div className="auth-header">
            <div>
              <p className="auth-title-kicker">{titles[view]}</p>
              <h3 className="auth-title">Secure access</h3>
            </div>
